'''laptop script. Leest BLE data van de Pico en rekent de risico's uit.'''
...
# instellingen
# beginwaarden, later aan te passen met commando's
AGE_YEARS = 22
...
def parse_float(text):
    '''zet tekst om naar float of None'''
    text = str(text).strip()
    if text == "":
        return None
    try:
...
def clamp(value, low, high):
    '''houdt waarde tussen min en max'''
    return max(low, min(high, value))


def format_duration(seconds):
    '''maakt seconden leesbaar'''
    return str(timedelta(seconds=int(seconds)))


def bell():
    '''piepje in de terminal'''
    print("\a", end="")


class RollingAverage:
    '''gemiddelde over de laatste paar seconden'''

    def __init__(self, window_seconds):
        '''maak een leeg venster'''
        self.window_seconds = window_seconds
        self.values = []

    def reset(self):
        '''gooi oude waarden weg'''
        self.values = []

    def add(self, t_s, value):
        '''voeg waarde toe en geef gemiddelde terug'''
    ...
    def average(self, t_s=None):
        '''bereken gemiddelde van wat er nog in zit'''
        if not self.values:
            return None
        return sum(v for _, v in self.values) / len(self.values)

    def oldest(self):
        '''pak de oudste waarde'''
        if not self.values:
            return None
...
# risico analyse
class RiskAnalyzer:
    '''houdt data bij en maakt de risicoscore'''

    def __init__(self, age_years=AGE_YEARS, skin_type=SKIN_TYPE):
        '''zet alle tellers en buffers klaar'''
    ...
    def set_skin_type(self, skin_type):
        '''huidtype aanpassen en UV resetten'''
    ...
    def dt_minutes(self, elapsed_s, now_wall):
        '''tijd tussen twee metingen in minuten'''
    ...
    def reapply_sunscreen_interval_min(self, uv_index, adjusted_heat_c, hr_relative, heavy_sweat_likely):
        '''bepalen wanneer opnieuw smeren moet'''
        if uv_index is None or uv_index < 3:
            return None
    ...
    def heat_index_c(self, temp_c, rh_pct):
        '''heat index in graden C berekenen'''
    ...
    def sun_correction_c(self, uv_index):
        '''extra warmte door zon erbij tellen'''
        if uv_index is None:
            return 0
        if uv_index < 3:
            return 0
    ...
    def environment_score(self, adjusted_heat_c):
        '''warmtebelasting naar score zetten'''
    ...
    def skin_temperature_score(self, skin_avg_c, elapsed_s):
        '''huidtemp naar score en trend zetten'''
    ...
    def duration_score(self, adjusted_heat_c, elapsed_s):
        '''score voor lang in warmte zijn'''
    ...
    def apply_hysteresis(self, raw_level_index, direct_escalation, now_wall):
        '''niet steeds van niveau laten springen'''
    ...
    def heavy_sweat_likely(self, hr_relative, adjusted_heat_c, rv_pct, fast_skin_rise):
        '''ruwe check of veel zweten logisch is'''
        return (
    ...
    def rest_recovery_poor(self, bpm_avg, hr_relative, now_wall):
        '''check of hartslag slecht zakt in rust'''
...
def parse_sensor_line(line):
    '''CSV regel van de Pico omzetten naar getallen'''
    line = line.strip()
...
def print_result(result):
    '''laat laatste status zien in de terminal'''
...
def _clean_xy(records, x_key, y_key):
    '''pak x en y uit de log voor grafieken'''
    xs, ys = [], []
    for r in records:
        x = r.get(x_key)
        y = r.get(y_key)
...
def build_message_events(records):
    '''maak een kort lijstje van meldingen'''
    events = []
    previous_key = None
...
def start_input_thread(cmd_queue):
    '''start input lezen naast BLE'''
    def worker():
        '''blijft commando regels lezen'''
        while True:
            try:
                cmd = input().strip()
                cmd_queue.put(cmd)
...
class SessionController:
    '''houdt bij of de laptop meting nog loopt'''

    def __init__(self):
        '''maak controller voor deze sessie'''
        self.stop_requested = False

    def handle_command(self, cmd, analyzer):
        '''voer terminal commando uit'''
        if not cmd:
            return
...
async def run_session():
    '''maak verbinding en draai de meting'''
    ...
    while True:
        ...
        def on_notify(sender, data):
            '''komt binnen als de Pico iets stuurt'''
